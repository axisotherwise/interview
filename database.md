...
1. 공유 락
읽기 작업 시 사용되는 락임. <br>
공유 락끼리는 동시에 접근이 가능하며 데이터를 읽는 도중 여러 사용자가 동시에 접근 가능함. <br>
...
### 11. 조인
2개 이상의 테이블을 결합하는 연산 작업을 의미함. <br>
데이터의 규모가 커지면서 하나의 테이블로 정보를 수용하기 어려워 테이블을 분할하고 관계성을 부여함. <br>

조인은 이너 조인과 아우터 조인으로 나뉨.
조인 작업 시 어느 테이블을 먼저 읽느냐에따라 연산량이 상당히 차이남.

이너 조인은 어느 테이블을 먼저 읽어도 달라지지 않으며 
옵티마이저가 조인의 순서를 조절해 다양한 방법으로 최적화를 수행할 수 있음

아우터 조인은 반드시 아우터가 되는 테이블을 먼저 읽어야하므로 조인 순서를
옵티마이저가 선택할 수 없음.

이너 조인에서는 교집합의 데이터를 모두 버리지만 아우터 조인에서는 일치하지않더라도
NULL로 채워 결과를 응답한다.

주의사항 <br>
데이터베이스 실행 계획은 이너 조인을 사용했는지 아우터 조인을 사용했는지
알려주지않으므로 아우터 조인을 의도한 쿼리가 이너 조인으로 실행되는지 않는지
주의해야함. <br>

레프트 조인의 ON 절에 모두 명시하는 게 좋음. <br>
그렇지 않으면 아우터 조인을 내부적으로 이너 조인으로 변경시켜 처리할 수도 있음. <br>
레프트 조인의 ON 절에 명시되는 조건은 조인되는 레코드가 있을 경우에만 적용됨. <br>

이너 조인은 조인 테이블 모두 레코드가 존재하는 경우 레코드가 반환됨. <br>
아우터 조인은 아우터 테이블에 레코드가 존재하면 레코드가 반환딤. <br>

1. Inner Join <br>
양쪽 모두 연관돼있는 데이터를 검색하는 방법임. <br>
A와 B에 수행되는 건 A와 B의 교집합을 의미함. <br>

2. Outer Join <br>
한 쪽에만 데이터가 있고 한 쪽에는 없는 경우 데이터가 있는 쪽의 내용을 전부 출력하는 조인 방법임. <br>
A와 B에 수행되는 건 A와 B의 합집합을 의미함. <br>

2-1. Left Outer Join <br>
왼쪽 테이블을 기준으로 오른쪽의 테이블을 매치 <br>
왼쪽 테이블의 한 개의 레코드의 여러 개의 오른쪽 레코드가 일치할 경우 해당 왼쪽 레코드를 여러 번 표시함. <br>
왼쪽은 무조건 표시하고 매치되는 레코드가 없으면 NULL을 표시함. <br>

2-2. Right Outer Join <br>
오른쪽 테이블을 기준으로 왼쪽에 테이브을 매치 <br>
오른쪽 테이블의 한 개의 레코드의 여러 개의 왼쪽 레코드가 일치할 경우 오른쪽 레코드를 여러 번 표시함. <br>
오른쪽은 무조건 표시하고 매치되는 레코드가 없으면 NULL을 표시함. <br>
...
장점 <br>
Key-Value 형태로 저장되므로 응답 속도가 빠름. <br>
