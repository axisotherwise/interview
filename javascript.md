...
복사된 데이터가 원본 객체와 다른 메모리 주소를 가리키며 복사본이 수정되어도 원본은 변경되지 않음. <dr>

<br>

### 5. 콜 스택 메모리 힙
1. 콜 스택 <br>
코드를 읽어내려가며 작업들을 관리하기 위한 메모리 공간임. <br>
메모리 힙에서 작업에 필요한 요소들을 찾고 작업은 후입선출의 스택 형태로 쌓임. <br>

* 원시 타입과 실행 컨텍스트가 저장됨. <br>

2. 메모리 힙 <br>
콜 스택의 최상단에 있는 실행 컨텍스트가 실행되면서 참조되는 객체들이 저장되어있는 메모리 공간임. <br>

참조 타입의 데이터가 저장되며 데이터의 크기는 런타임 단계에서 동적으로 결정되므로 힙은 구조화되어있지 않음. <br>

<br>

### 6. 이벤트 루프
논 블로킹 비동기 작업을 가능하게 해주는 시스템임. <br>
...
var, function, import 구문은 호이스팅되므로 TDZ에 영향 없음. <br>
...
### 16. 호이스팅
코드 실행 시 변수와 함수의 선언을 스코프 최상단으로 끌어올리는 걸 의미함. <br>
var와 함수 선언문에서 발생하며 let과 const 함수 표현식에서는 발생하지 않음. <br>

var는 선언과 동시에 초기화가 이루어지므로 호이스팅 시 초기화도 이루어지며 undefined라는 값이 할당됨. <br>
