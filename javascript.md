...
* 값이 복사되어 전달되는 Call By Value

<br>

2. 참조 타입 <br>
가변 값이며 데이터 크기는 정해져 있지 않고 변수에 할당되면 직접 변수에 저장되는 게 아니라 데이터에 대한 참조만 저장됩니다. <br>

* 참조는 데이터의 주소이지 값이 아님. <br>
* 실제 값이 아닌 참조값이 전달되는 Call By Reference
...
### 얕은 복사 깊은 복사
1. 얕은 복사 <br>
참조 타입 데이터가 저장한 메모리의 주소값을 복사합니다. <br>
데이터 복사 시 원본 값과 복사된 값이 같은 메모리 주소를 가리키며 복사 후 변수를 재사용해 수정한다면 원본의 값도 변경됩니다. <br>
...
2. 깊은 복사 <br>
복사된 데이터가 원본과 다른 메모리 주소를 참조하며 내부의 값만 복사됩니다. <br>
...
### 콜 스택 메모리 힙
1. 콜 스택 <br>
코드를 읽어내려가며 작업들을 관리하기 위한 메모리 공간입니다. <br>
작업에 필요한 요소들을 메모리 힙에서 찾고 작업은 스택 형태로 쌓입니다. <Br>

* 원시 타입과 실행 컨텍스트가 저장됩니다. <br>

<br>

2. 메모리 힙 <br>
크기가 동적으로 변하는 참조 타입의 값이 저장되는 공간입니다. <br>
...
### 콜백 지옥
콟개 함수가 반복되어 코드의 들여 쓰기 수준이 감당하기 힘들 정도로 깊어지는 현상을 의미합니다. <br>

### this
함수가 호출되는 방식에 따라 동적으로 변하는 자기 참조 변수입니다. <br>
...
명시적으로 없음을 나타내야한다면 null을 사용해야 합니다. <br>

* typeOf null은 object인데 이 현상은 자바스크립트 설계 에러이며 일치 연산자를 활용해 검사해야합니다. <br>
